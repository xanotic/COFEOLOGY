...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action'])) {
  if ($_POST['action'] === 'toggle_availability') {
      ...
      $stmt = $conn->prepare("UPDATE MENU_ITEM SET AVAILABILITY = ? WHERE ITEM_ID = ?");
...
// Get all menu items with error handling
...
if (!empty($menuItems)) {
    ...
    // Uncomment this line to see what columns exist in your table
    // echo "<pre>Available columns: " . print_r(array_keys($firstItem), true) . "</pre>";
}
...
$activeItems = count(array_filter($menuItems, function($item) { 
    return isset($item['AVAILABILITY']) ? $item['AVAILABILITY'] : 1; 
}));
